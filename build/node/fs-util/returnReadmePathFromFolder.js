 ...
 */
export const returnReadmePathFromFolder = async (absoluteFolderPath) => {
    const dirents = await fs.readdir(absoluteFolderPath, {
        withFileTypes: true,
        encoding: "utf8",
    });
    const readmeDirent = dirents.find((x) => x.isFile() && x.name.toLowerCase() === "readme.md");
    if (!readmeDirent)
        return;
    const absoluteReadmeFilePath = path.join(absoluteFolderPath, readmeDirent.name);
