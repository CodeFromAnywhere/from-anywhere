 ...
 */
export const downloadFile = async (url, absoluteOutputFolderPath, customName, isOverwrite, defaultExtension, isNoJson, isNoText, maxTimeoutMs) => {
    ...
    const { response, status, statusText } = await fetchWithTimeout(url, undefined, 30000, isNoJson, isNoText);
    ...
    const { result: arrayBuffer } = await runFunctionWithTimeout(async () => {
    ...
    const { result: resultOutputPath } = await runFunctionWithTimeout(async () => {
