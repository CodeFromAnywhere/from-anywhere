 ...
 */
const test = async () => {
    const folder = "/Users/king/os/packages/_actionschema/os-web/public/domains/karsens.com/blogs-2016-2022";
    const files = fs.readdirSync(folder);
    const nonPublished = (await Promise.all(files.map(async (name) => {
    ...
    await Promise.all(nonPublished.map((p) => {
        return fs.rename(p, path.join("/Users/king/os/packages/control-fs/fs-util/assets", path.parse(p).base));
    }));
