 ...
 */
export const isIndexableFileId = (fileId: string): boolean => {
  const isOperationName = hasSpecificSubExtension(
    fileId,
    operationUnindexableNamesOrSubExtensions,
    true,
  );

  const isFrontendOptionalName = hasSpecificSubExtension(
    fileId,
    frontendOptionalFileSubExtensions,
    false,
  );

  const isTypescriptDeclarationFile = hasSpecificSubExtension(
    fileId,
    ["d"],
    false,
  );
