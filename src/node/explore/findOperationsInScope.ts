 ...
 * Efficient!
 */
export const findOperationsInScope = async (
  absolutePath: string,
  ...
  const firstPackageFolderPath = findFolderWhereMatch(absolutePath, (p) =>
  ...
  const packageJson = await readJsonFile<Operation>(
    path.join(firstPackageFolderPath, "package.json"),
  );
  ...
  const packageFolderPaths = (
    await findAllPackages({ basePath: absolutePath })
  ).map((x) => path.parse(x.path).dir);
