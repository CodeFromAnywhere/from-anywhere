 ...
 */
export const getPathCombinations = (
  ...
  const realChunkSegmentArrays = chunkSegmentArrays.map((chunks) =>
    chunks.filter(notEmpty),
  );

  return realChunkSegmentArrays.reduce(
    (cumSegments, chunks) => {
...
/**
   ...
   */
export const getFolder = (pathString: string) => {
 ...
 * output: folder
 */
export const getLastFolder = (pathString: string) => {
